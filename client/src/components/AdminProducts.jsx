...
// Moderación de productos publicados: el administrador puede retirar los que no correspondan
function AdminProducts() {
    const { products, pagination, page, setPage, status, error, reload, removal } = useAdminProducts();
                ...
                <ul className="list-group">
                    {products.map((product) => (
                        <li key={product.id} className="list-group-item d-flex flex-column flex-sm-row justify-content-between gap-2">
                            ...
                            <button type="button" className="btn btn-outline-danger align-self-sm-center" onClick={() => removal.open(product)}>
