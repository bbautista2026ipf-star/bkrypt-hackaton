...
// Sin filtros se muestran todas las ferias con horarios próximos. Con filtros, cada feria conserva solo a los emprendedores
...
const loadActiveFairs = async (productFilters) => {
    const [{ schedules }, matchingProducts] = await Promise.all([
        getSchedules(),
    ...
    const matchingEntrepreneurIds = matchingProducts
        ...
        : null;
    return buildActiveFairs(schedules, matchingEntrepreneurIds);
};
